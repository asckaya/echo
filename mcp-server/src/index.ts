...
const LanguageEnum = z
  .enum(['en', 'zh'])
  .default('en')
  .describe("Content language. 'en' writes to content/, 'zh' writes to content/zh/")
...
function listContentFiles(language: Language = 'en'): {
  json: string[]
...
const MarkdownCategoryEnum = z.enum(['projects', 'publications', 'articles', 'about'])

const ProjectCategoryEnum = z.enum(['robotics', 'nlp', 'web-app', 'data', 'tooling', 'healthcare'])
    ...
    if (!fs.existsSync(fullPath)) {
      return {
        content: [
          {
            type: 'text' as const,
            text: `Error: File not found: ${getContentRelPath(lang, file_path)}`,
          },
        ],
        isError: true,
      }
...
server.tool(
    ...
    "For about.md, use category='about'. Use language='zh' to write Chinese version.",
  {
    ...
    slug: z
      ...
      .optional()
      .describe(
        "Filename slug (without .md). Auto-generated from title if omitted. Ignored for 'about'.",
      ),
  ...
  async ({ category, slug, frontmatter, body, language }) => {
    try {
      ...
      return {
        content: [{ type: 'text' as const, text: `Successfully wrote ${relPath}` }],
      }
    } catch (e) {
      return {
        content: [
...
server.tool(
  ...
  async ({ file_path, language }) => {
    ...
    if (!fs.existsSync(fullPath)) {
      return {
        content: [
          { type: 'text' as const, text: `File not found: ${getContentRelPath(lang, file_path)}` },
        ],
        isError: true,
      }
    }
...
server.tool(
  'add_experience',
  'Add a timeline entry to experience.json. ' + "Use language='zh' to add to Chinese content.",
  {
    ...
    category: z
      .enum(['research', 'industry', 'academic', 'leadership'])
      .describe('Experience category'),
    role_type: z
      .enum(['research', 'mle', 'sde', 'teaching', 'leadership'])
      .optional()
      .describe('Role type'),
...
server.tool(
  'add_education',
  'Add an education entry to experience.json. ' + "Use language='zh' to add to Chinese content.",
  {
...
server.tool(
  ...
  async (params) => {
    ...
    return {
      content: [{ type: 'text' as const, text: `Added news item (${lang}): ${params.title}` }],
    }
  },
)
...
server.tool(
  ...
  async (params) => {
    ...
    return {
      content: [{ type: 'text' as const, text: `Added award (${lang}): ${params.title}` }],
    }
  },
)
...
server.tool(
  ...
  "Add a talk/presentation to talks.json. Use language='zh' to add to Chinese content.",
  {
    ...
    location: z.string().optional().describe('Location'),
    type: z
      .enum(['keynote', 'invited', 'oral', 'poster', 'tutorial', 'workshop', 'panel', 'other'])
      .optional()
      .describe('Talk type'),
  ...
  async (params) => {
    ...
    return {
      content: [{ type: 'text' as const, text: `Added talk (${lang}): ${params.title}` }],
    }
  },
)
      ...
      sections_to_extract: [
        {
          ...
          tool: 'add_education',
          hint:
            'Extract degree, institution, year range for each education entry. ' +
            (params.languages.includes('zh')
              ? "For zh: translate degree names (e.g. '计算机科学硕士')."
              : ''),
        },
        {
          ...
          tool: 'add_experience',
          hint:
            'Extract title, company, dates, highlights for each position. ' +
            'Set category: research|industry|academic|leadership, roleType: research|mle|sde|teaching|leadership. ' +
            (params.languages.includes('zh')
              ? 'For zh: translate job titles, summaries, and highlights.'
              : ''),
        },
        {
          ...
          tool: 'add_publication',
          hint:
            'Extract title, authors, venue, year, status, links for each paper. ' +
            "Generate a unique ID like 'venue2025-short-title'. " +
            (params.languages.includes('zh')
              ? 'For zh: translate abstract. Keep title/venue/authors in English.'
              : ''),
        },
        {
          section: 'Projects',
          tool: 'add_project',
          hint:
            'Extract title, summary, tags, category, highlights for each project. ' +
        ...
        },
        {
          ...
          tool: 'add_award',
          hint:
            'Extract title, org, date, kind for each award. ' +
        ...
        },
        {
          ...
          hint:
            'Extract current research labs/groups with advisor, focus, link. ' +
            (params.languages.includes('zh')
              ? 'For zh: translate research focus descriptions.'
              : ''),
        },
        {
          section: 'About / Bio',
          ...
          hint:
            'Generate a narrative journey description and timeline phases from the resume. ' +
            (params.languages.includes('zh')
              ? 'For zh: write a Chinese version of the bio narrative.'
              : ''),
        },
        {
          ...
          tool: 'add_talk',
          hint:
            'Extract title, event, date, location, type for each talk or presentation. ' +
            (params.languages.includes('zh')
              ? 'For zh: translate description. Keep event names in English.'
              : ''),
        },
        {
          section: 'Teaching',
          tool: 'add_teaching',
          hint:
            'Extract course, institution, semester, role for each teaching entry. ' +
            (params.languages.includes('zh')
              ? 'For zh: translate course names and descriptions.'
              : ''),
        },
      ],
...
server.tool(
  ...
  'Copy images or other assets to the public directory for use in the portfolio',
  {
    ...
    source_path: z.string().optional().describe("Absolute path to source file (for 'copy' action)"),
    destination: z
      .enum(['img', 'logos', 'project-images', 'publication-images', 'files'])
      .optional()
      .describe('Destination subdirectory in public/'),
    filename: z.string().optional().describe('Target filename (defaults to source filename)'),
  },
  async ({ action, source_path, destination, filename }) => {
    ...
    if (!fs.existsSync(source_path)) {
      return {
        content: [{ type: 'text' as const, text: `Error: Source file not found: ${source_path}` }],
        isError: true,
      }
    }
...
server.tool(
  ...
  'Build or start the TermHub dev server for preview',
  {
    action: z
      .enum(['dev', 'build', 'stop'])
      .describe(
        "'dev' starts dev server, 'build' creates production build, 'stop' stops dev server",
      ),
  },
  async ({ action }) => {
    try {
      ...
      if (devServerProcess) {
        return {
          content: [{ type: 'text' as const, text: 'Dev server is already running.' }],
        }
      }
          ...
          text: JSON.stringify(
            {
              ...
              usage: {
                ...
                components:
                  'Set "components": { "hero": "<variantId>" } to override individual sections.',
              },
            },
            null,
            2,
